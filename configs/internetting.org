...
**** Ignore some email addresses
     ...
     email addresses in things like gnus. I'll likely want to iterate
     on this, but let's ignore everything with "noreply" in it to
     start.

     #+BEGIN_SRC emacs-lisp
       (defun gds-ebdb-check-email-address (record address)
	 "Check if ADDRESS is an email address we want to record in EBDB.

       Returns either t or nil. Intended to be used in 'ebdb-add-mails'"
	 (not (string-match-p (regexp-quote "noreply") address)))

       (setq ebdb-add-mails #'gds-ebdb-check-email-address)
     #+END_SRC
