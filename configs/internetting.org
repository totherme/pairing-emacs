...
** Email
   ...
   I like using [[http://gnus.org/][gnus]] for email. To make this work, we'll need some
	...
	(lambda ()
	  (let* ((creds (json-read-from-string (gds-lastpass-get-note "Personal/email-creds")))
	    ...
	    (setq smtpmail-smtp-server smtp-address
		  smtpmail-smtp-service smtp-port	;gdsgds this might need to be a number not a string
	    ...
	    (cl-defun gds-gnus-auth-source-search (&rest spec
							 ...
							 &allow-other-keys)
	      (let ((host-address (cadr host))) ; Gnus asks for passwords
						; with host of the form
