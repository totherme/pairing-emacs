...
*** TODO Make using callbacks nicer
    At the moment when I use ~gds-lastpass-ensure-logged-in-and-then~,
    I have to explicitly wrap the callback in a ~lambda~. It might be
    prettier to not have to do this -- to make it a form more like
    ~if~. This might mean writing a macro...?
...
** Slack
   ...
   Create a lastpass entry at location =Personal/slack-creds=, and put
   ...
   interested in (which may or may not be the [[https://emacs.slack.com/][emacs slack]]), and to use
   your own ID, secret, and token. Unfortunately you can't use use
   OAuth or give it your regular password, so getting those
   credentials is a little involved. [[https://github.com/yuya373/emacs-slack#how-to-get-token-the-easy-way][Read how here]].

   Once your creds are in lastpass, emacs-slack should [[https://github.com/yuya373/emacs-slack/wiki/ScreenShots][work as advertised]].
...
*** TODO BUG: the config runs /slightly/ too late
    When I run =M-x slack-start= for the first time, the package
    loads, the slack command starts running, and only then does the
    config start running. This means that we get simultaneously asked
    for a slack team name (because slack doesn't have out config from
    lastpass yet) and for a lastpass password. We have to abort the
    first slack login, go through the lastpass business, and then
    re-start the slack command.

    Possibly the easiest fix is to add the config as advice around the
    ~slack-start~ command? We'd want it to run only once per session,
    but it can presumably check if a slack team has been registered
    yet to do that.
